...
Computes entropy, compression, autocorrelation, spectral, AR(2), motif metrics,
and a composite structure score. Also includes null-model generation and bootstrap
CI for comparing real vs null structure.
"""
...
# ---------------------------------------------------------------------------
# Step-response metrics (for control/image signals)
# ---------------------------------------------------------------------------
...
def step_metrics(
    ...
    # Initial slope via linear regression over first 5%
    n5 = max(2, int(0.05 * n))
    t5, y5 = t[:n5], y[:n5]
...
# ---------------------------------------------------------------------------
# Null model generators and bootstrap
# ---------------------------------------------------------------------------

def _phase_scramble(x: np.ndarray, seed: int) -> np.ndarray:
    rng = np.random.default_rng(seed)
    y = np.fft.rfft(x - np.mean(x))
    y2 = y.copy()
    if len(y2) > 2:
        ph = rng.uniform(0, 2 * math.pi, len(y2) - 2)
        y2[1:-1] = np.abs(y[1:-1]) * np.exp(1j * ph)
    return _norm(np.fft.irfft(y2, n=len(x)))
...
def generate_null(x: np.ndarray, seed: int, kind: str) -> np.ndarray:
    ...
    if kind == "phase_scramble":
        return _phase_scramble(x, seed + 20_000_000)
