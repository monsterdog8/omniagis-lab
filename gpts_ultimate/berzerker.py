"""
BERZERKER Omega — Decoupled Ablation Engine
...
def simulate(
    ...
    """Simulate coupled logistic lattice. Returns (steps-transient, N) array."""
    rng = np.random.default_rng(seed)
...
def metrics_from(hist: np.ndarray, ablate: Optional[int] = None) -> Dict[str, float]:
    ...
    if A.shape[1] < 2:
        ...
        coh = float(np.nanmean(np.abs(Cm[iu])))

    d = np.diff(s)
    denom = float(np.sum(np.abs(d)))
    direc = float(abs(float(np.sum(d)))) / denom if denom > 1e-9 else 0.0

    late = s[-max(1, len(s) // 3):]
    attr = 1.0 / (1.0 + float(np.var(late)) * 100)

    return dict(memory=mem, coherence=coh, direction=direc, attractor=attr)
...
def null_threshold(
    ...
    """95th percentile of |effect| under permuted coupling — empirical, not hardcoded."""
