"""Claim classification and evidence scoring.

Classifies text claims as blocked/bounded/unknown using regex pattern matching.
Computes a weighted evidence score (DATA, RAW, SCORING, REPLAY, INDEPENDENCE, SAFETY)
with fail-closed penalties — score stays at zero unless all required gates pass.
"""
...
def compute_evidence_score(inp: EvidenceInput) -> EvidenceScore:
    ...
    Returns a score of 0.0 unless RAW outputs are present and valid.
    Public claims require final_score >= 0.85 AND all dimensions > 0.
    """
...
def proof_firewall(dims: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Final public-claim gate. Returns BLOCKED unless all proof dimensions are non-zero and SAFETY >= 0.75."""
    dims = dims or {}
